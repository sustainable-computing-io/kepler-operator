...
var _ = Describe("PowerMonitorInternal Secrets", Serial, func() {
	...
	Describe("Single Secret Mount", func() {
		It("should handle missing secret then recover when created", func() {
			...
			By("creating PowerMonitorInternal with reference to non-existent secret")
			...
			By("verifying PowerMonitorInternal is degraded due to missing secret")
			...
			By("waiting for PowerMonitorInternal to be reconciled after secret creation")
			pmi = f.ExpectPowerMonitorInternalCondition(name, v1alpha1.Reconciled, v1alpha1.ConditionTrue)

			By("verifying secret volume is added")
			secretVolumes := 0
	...
	Describe("Secrets Lifecycle CRUD", func() {
		It("should handle complete secret lifecycle correctly", func() {
			...
			By("creating PowerMonitorInternal without any secrets")
			pmi := f.CreateTestPowerMonitorInternal(name, testNs, runningOnVM, testKeplerImage, testKubeRbacProxyImage, Cluster, v1alpha1.SecurityModeNone, []string{})
			...
			By("adding a secret reference to PMI (secret doesn't exist yet)")
			secretRef := v1alpha1.SecretRef{
				Name:      secretName,
			...
			By("waiting for PMI to become degraded due to missing secret")
			...
			By("removing the secret reference from PMI spec")
			pmi = f.GetPowerMonitorInternal(name)
			{
			...
			By("verifying the Available condition is healthy again")
			finalCondition, err := k8s.FindCondition(pmi.Status.Conditions, v1alpha1.Available)
			Expect(err).NotTo(HaveOccurred(), "Should find Available condition")
			Expect(finalCondition.Reason).NotTo(Equal(v1alpha1.SecretNotFound),
	...
	Describe("Auto Redeploy On Secret Change", func() {
		It("should redeploy when secret content changes", func() {
			...
			By("creating PowerMonitorInternal with secret reference")
			pmi := f.CreateTestPowerMonitorInternal(name, testNs, runningOnVM, testKeplerImage, testKubeRbacProxyImage, Cluster, v1alpha1.SecurityModeNone, []string{},
				utils.PowerMonitorInternalBuilder{}.WithSecrets([]v1alpha1.SecretRef{secretRef}),
			)
			...
			By("creating the initial secret")
			...
			By("updating the secret with different content")
			...
			By("waiting for the controller to detect the secret change")
			...
			By("verifying the DaemonSet was updated with new secret hash")
			updatedDs := &appsv1.DaemonSet{}
