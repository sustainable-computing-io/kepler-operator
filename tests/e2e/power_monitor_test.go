...
var _ = Describe("PowerMonitor", Serial, func() {
	...
	Describe("Deletion", func() {
		It("should delete PowerMonitor successfully", func() {
			By("verifying PowerMonitor doesn't exist initially")
			f.ExpectNoResourceExists("power-monitor", "", &v1alpha1.PowerMonitor{})

			By("creating PowerMonitor")
			f.CreateTestPowerMonitor("power-monitor", runningOnVM)

			By("waiting until PowerMonitor is available")
	...
	Describe("Reconciliation", func() {
		It("should reconcile PowerMonitor and create DaemonSet", func() {
			...
			By("verifying default toleration and reconciled condition")
	...
	Describe("Taints and Tolerations", func() {
		It("should deploy with correct tolerations", func() {
			...
			// Taint a node
			nodes := f.GetSchedulableNodes()
			Expect(nodes).NotTo(BeEmpty(), "got zero schedulable nodes")
			node := nodes[0]
	...
	Describe("AdditionalConfigMap", func() {
		It("should reconcile with additional config map", func() {
			...
			// Verify reconciliation fails without config map
  ...
  fake-cpu-meter:
	...
	Describe("RBAC Reconciliation", func() {
		It("should reconcile with RBAC security mode", func() {
			...
			if Cluster == k8s.Kubernetes {
				...
				f.ExpectPowerMonitorCondition(pm.Name, v1alpha1.Reconciled, v1alpha1.ConditionTrue)
	...
	Describe("Security", func() {
		It("should have correct pod and container security context", func() {
			...
			By("verifying pod-level security settings")
			Expect(ds.Spec.Template.Spec.HostPID).To(BeTrue(), "hostPID should be enabled for /proc access")
			Expect(ds.Spec.Template.Spec.HostNetwork).To(BeFalse(), "hostNetwork should be disabled")

			By("verifying Kepler container security context")
			...
			Expect(*secCtx.Privileged).To(BeFalse(), "Container should not be privileged")
			Expect(*secCtx.AllowPrivilegeEscalation).To(BeFalse(), "Should not allow privilege escalation")
			Expect(*secCtx.ReadOnlyRootFilesystem).To(BeTrue(), "Root filesystem should be read-only")
			Expect(secCtx.Capabilities).NotTo(BeNil(), "Container should have capabilities set")
			Expect(secCtx.Capabilities.Drop).To(Equal([]corev1.Capability{"ALL"}),
				"Should drop ALL capabilities")
			Expect(secCtx.Capabilities.Add).To(Equal([]corev1.Capability{"SYS_PTRACE"}),
				"Should only add SYS_PTRACE capability")
		})
	})
	...
	Describe("Secrets Lifecycle CRUD", func() {
		It("should handle secret lifecycle correctly", func() {
			...
			By("creating PowerMonitor without any secrets")
			...
			By("adding a secret reference to PowerMonitor (secret doesn't exist yet)")
			...
			By("waiting for PowerMonitor to become degraded due to missing secret")
			...
			By("creating the missing secret and waiting for recovery")
			...
			By("verifying the secret is properly mounted in the DaemonSet")
			ds := &appsv1.DaemonSet{}
			...
			By("deleting the secret to trigger degradation again")
			...
			By("removing the secret reference from PowerMonitor spec")
			pm = f.GetPowerMonitor(name)
			{
