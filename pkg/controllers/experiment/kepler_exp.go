...
func exporterReconcilers(k *v1alpha1.KeplerExperiment, cluster k8s.Cluster) []reconciler.Reconciler {
	...
	if cleanup := !k.DeletionTimestamp.IsZero(); cleanup {
		...
		if cluster == k8s.OpenShift {
			rs = append(rs,
				resourceDeleter(exporter.NewSCC(components.Metadata)),
			)
		}
		return rs
	}
	...
	rs := updatersForResources(k,
		...
		exporter.NewConfigMap(components.Full, deployment),
		exporter.NewDaemonSet(components.Full, deployment),
		exporter.NewService(deployment),
