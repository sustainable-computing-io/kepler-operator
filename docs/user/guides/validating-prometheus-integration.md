...
If CRD doesn't exist, prometheus-operator is not installed. Install it:
...
Save this as `validate-prometheus.sh`:
...
echo "=== Kepler Prometheus Integration Validation ==="
echo
...
echo "Step 1: Checking ServiceMonitor..."
if kubectl get servicemonitor power-monitor -n power-monitor &>/dev/null; then
...
echo
echo "Step 2: Checking Kepler Service..."
if kubectl get svc power-monitor -n power-monitor &>/dev/null; then
    check_pass "Service exists"

    # Check endpoints
    ENDPOINTS=$(kubectl get endpoints power-monitor -n power-monitor -o jsonpath='{.subsets[0].addresses[*].ip}')
    if [ -n "$ENDPOINTS" ]; then
        check_pass "Service has endpoints: $ENDPOINTS"
    else
        check_fail "Service has no endpoints"
    fi
else
...
# Step 3: Check Prometheus discovery
echo
echo "Step 3: Checking Prometheus discovery..."
PROM_POD=$(kubectl get pods -n monitoring -l app.kubernetes.io/name=prometheus -o jsonpath='{.items[0].metadata.name}' 2>/dev/null)
if [ -n "$PROM_POD" ]; then
    if kubectl logs -n monitoring "$PROM_POD" prometheus 2>/dev/null | grep -q "serviceMonitor/power-monitor"; then
        check_pass "Prometheus discovered ServiceMonitor"
    else
        check_warn "ServiceMonitor may not be discovered yet (check Prometheus logs)"
    fi
else
    check_warn "Prometheus pod not found in monitoring namespace"
fi

# Step 4: Check target health (requires port-forward)
echo
echo "Step 4: Checking Prometheus target health..."
...
# Check if Kepler target exists and is healthy
TARGET_HEALTH=$(curl -s http://localhost:9090/api/v1/targets 2>/dev/null | \
    jq -r '.data.activeTargets[] | select(.labels.job == "power-monitor") | .health' 2>/dev/null)

if [ "$TARGET_HEALTH" = "up" ]; then
    check_pass "Prometheus target is healthy (up)"
else
    check_fail "Prometheus target is not healthy: $TARGET_HEALTH"
fi

# Step 5: Check metrics availability
echo
echo "Step 5: Checking Kepler metrics in Prometheus..."
METRIC_COUNT=$(curl -s 'http://localhost:9090/api/v1/label/__name__/values' 2>/dev/null | \
    jq -r '.data[]' 2>/dev/null | grep -c ^kepler_ || echo 0)

if [ "$METRIC_COUNT" -gt 0 ]; then
    check_pass "Found $METRIC_COUNT Kepler metrics in Prometheus"

    # Sample a specific metric
    SAMPLE_VALUE=$(curl -s 'http://localhost:9090/api/v1/query?query=kepler_node_cpu_watts' 2>/dev/null | \
        jq -r '.data.result[0].value[1]' 2>/dev/null)

    if [ -n "$SAMPLE_VALUE" ] && [ "$SAMPLE_VALUE" != "null" ]; then
        check_pass "Sample metric value: kepler_node_cpu_watts = $SAMPLE_VALUE watts"
    fi
else
    check_fail "No Kepler metrics found in Prometheus"
fi
...
```bash
chmod +x validate-prometheus.sh
...
```text
=== Kepler Prometheus Integration Validation ===

Step 1: Checking ServiceMonitor...
✓ ServiceMonitor exists

Step 2: Checking Kepler Service...
✓ Service exists
✓ Service has endpoints: 10.244.0.10

Step 3: Checking Prometheus discovery...
✓ Prometheus discovered ServiceMonitor

Step 4: Checking Prometheus target health...
  (Starting port-forward to Prometheus...)
✓ Prometheus target is healthy (up)

Step 5: Checking Kepler metrics in Prometheus...
✓ Found 11 Kepler metrics in Prometheus
✓ Sample metric value: kepler_node_cpu_watts = 12.45 watts
...
- **[Configure PowerMonitor](../configuring-kepler.md)** to adjust metric levels and collection settings
