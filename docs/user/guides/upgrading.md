...
```bash
kubectl rollout status deployment/kepler-operator-controller-manager -n kepler-operator
```
...
2. **Verify operator version**:
   ...
   ```bash
   kubectl get deployment kepler-operator-controller-manager -n kepler-operator -o jsonpath='{.spec.template.spec.containers[0].image}'
   ```
...
```bash
# Check operator logs
kubectl logs -n kepler-operator deployment/kepler-operator-controller-manager --tail=50
...
```bash
kubectl get powermonitor -A -o wide
```

Check that all PowerMonitors show healthy status:
...
2. Manually trigger reconciliation by adding an annotation:
   ...
   ```bash
   kubectl annotate powermonitor <name> \
...
- **[Configuration Guide](./configuration.md)** - Review configuration options for new versions
