...
1. **Backup PowerMonitor resources** (if you might reinstall later):
   ...
   ```bash
   kubectl get powermonitor -A -o yaml > powermonitor-backup.yaml
   ```
...
Delete all PowerMonitor instances first:

```bash
# List all PowerMonitors
kubectl get powermonitor -A

# Delete specific PowerMonitor
kubectl delete powermonitor <name> -n <namespace>

# Or delete all PowerMonitors
kubectl delete powermonitor --all -A
```
...
1. **Delete PowerMonitor Resources**:
   ...
   - Delete all PowerMonitor instances

   Or via CLI:

   ```bash
   oc delete powermonitor --all -A
   ```
...
Wait for all Kepler DaemonSets to be removed.
...
oc delete crd keplers.kepler.system.sustainable.computing.io
oc delete crd keplersinternals.kepler.system.sustainable.computing.io
```
...
1. **Check for finalizers**:
   ...
   ```bash
   kubectl get powermonitor <name> -n <namespace> -o yaml | grep finalizers
   ```

   Remove finalizers if stuck:

   ```bash
   kubectl patch powermonitor <name> -n <namespace> \
     --type json \
...
2. **Force delete**:
   ...
   ```bash
   kubectl delete powermonitor <name> -n <namespace> --grace-period=0 --force
   ```
