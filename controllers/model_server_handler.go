...
func (msd *ModelServerDeployment) buildModelServerConfigMap() corev1.ConfigMap {
	...
	if modelServerExporter != nil {
		...
		if dataPairing["MODEL_PATH"] == "" {
			dataPairing["MODEL_PATH"] = "models"
		}
		...
		dataPairing["MODEL_SERVER_REQ_PATH"] = modelServerExporter.ModelServerRequiredPath
		if dataPairing["MODEL_SERVER_REQ_PATH"] == "" {
			dataPairing["MODEL_SERVER_REQ_PATH"] = "/model"
		}
	}
	if modelServerTrainer != nil {
		...
		if dataPairing["INITIAL_MODELS_LOC"] == "" {
			dataPairing["INITIAL_MODELS_LOC"] = "https://raw.githubusercontent.com/sustainable-computing-io/kepler-model-server/main/tests/test_models"
		}
	...
	configMap := corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{
			Name:      ModelServerConfigMapNameSuffix,
...
func (msd *ModelServerDeployment) buildModelServerService() corev1.Service {
	...
	modelServerService := corev1.Service{
		ObjectMeta: metav1.ObjectMeta{
			Name:      ModelServerServiceNameSuffix,
			Namespace: msd.Instance.Namespace,
			Labels:    labels,
		},
...
func (msd *ModelServerDeployment) buildModelServerDeployment() appsv1.Deployment {
	...
	deployment := appsv1.Deployment{
		ObjectMeta: metav1.ObjectMeta{
			Name:      ModelServerDeploymentNameSuffix,
			Namespace: msd.Instance.Namespace,
			Labels:    labels,
		},
...
func (msd *ModelServerDeployment) ensureModelServerPersistentVolumeClaim(l klog.Logger) (bool, error) { //(ReconciliationResult, error) {
	...
	err := msd.Client.Get(msd.Context, types.NamespacedName{Name: ModelServerPersistentVolumeClaimNameSuffix, Namespace: msd.Instance.Namespace}, msPVCResult)
	if err != nil {
