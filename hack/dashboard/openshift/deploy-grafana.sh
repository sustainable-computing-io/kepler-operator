...
oc apply --kustomize $(pwd)/hack/dashboard/openshift/
while ! oc get grafana --all-namespaces
do
    echo waiting for grafana custom resource definition to register
    sleep 5
done
oc apply -f $(pwd)/hack/dashboard/openshift/02-grafana-instance.yaml
oc apply -f $(pwd)/hack/dashboard/openshift/02-grafana-sa.yaml
oc apply -f $(pwd)/hack/dashboard/openshift/03-grafana-sa-token-secret.yaml

SERVICE_ACCOUNT=grafana-serviceaccount
SECRET=grafana-sa-token

while ! oc get serviceaccount $SERVICE_ACCOUNT -n kepler-operator-system
do
    sleep 2
done
# Define Prometheus datasource
oc adm policy add-cluster-role-to-user cluster-monitoring-view -z $SERVICE_ACCOUNT -n kepler-operator-system


export BEARER_TOKEN=$(oc get secret ${SECRET} -o json -n kepler-operator-system | jq -Mr '.data.token' | base64 -d) || or true
...
envsubst < $(pwd)/hack/dashboard/openshift/03-grafana-datasource-UPDATETHIS.yaml | oc apply -f -
...
oc apply -f $(pwd)/hack/dashboard/openshift/04-grafana-dashboard.yaml
